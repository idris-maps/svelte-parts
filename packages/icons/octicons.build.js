...
const IN_DIR = path.resolve(__dirname, 'temp', 'octicons')
const OUT_DIR = path.resolve(__dirname, 'octicons')

const removeExtension = d => d.split('-24.')[0]
...
const prepareTemplate = async () => {
  const template = await read(path.resolve(__dirname, 'octicons.template.html'))
...
const createComponent = template => async fileName => {
  ...
  const [_, part1] = file.split('viewBox="0 0 24 24">')
  const [content] = part1.split('</svg>')
...
const run = async () => {
  ...
  const files = (await readdir(IN_DIR)).filter(d => d.includes('-24.svg'))

  await Promise.all(files.map(createComponent(template)))
  await createIndex(files)

  console.log('Done creating octicons icons')
}
